...
If governance requirements cannot be verified:
...
Required authorization attributes must be explicitly defined by repository governance documents.

Authorization attributes must include:

- A repository authorization identifier
- A repository authorization token
- A repository permission classification
...
If any required authorization attribute is missing:

Assume governance validation failed.

If governance validation succeeds:

1. Determine the effective repository permissions.
...
If the effective repository permissions cannot be determined:
